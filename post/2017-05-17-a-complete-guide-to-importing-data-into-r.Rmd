...
- You can use `read_delim()` and specify the separator using `delim` to read data from any delimited file. 
- CSV (comma separated values) is a very popular file format. We can read data from CSV files using `read_csv()`. 
...
- If the number of whitespace characters between columns is not fixed or th lines are of different length, use `read_table2()`.
